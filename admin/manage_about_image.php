...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['activate_image'])) {
            ...
            $conn->query("UPDATE about_images SET is_active = FALSE");
            
            // Then activate the selected one
            $stmt = $conn->prepare("UPDATE about_images SET is_active = TRUE WHERE id = ?");
            $stmt->bind_param("i", $imageId);
            $stmt->execute();
            
            $_SESSION['message'] = "Image activated successfully!";
        }
        ...
        if (isset($_POST['delete_image'])) {
            ...
            $result = $conn->query("SELECT image_path FROM about_images WHERE id = $imageId");
            if ($result->num_rows > 0) {
                $image = $result->fetch_assoc();
                $filePath = '../' . $image['image_path'];
                
                // Delete from database
                $conn->query("DELETE FROM about_images WHERE id = $imageId");
...
// Get all images
$images = $conn->query("SELECT * FROM about_images ORDER BY uploaded_at DESC");
// Get active image
$activeImage = $conn->query("SELECT * FROM about_images WHERE is_active = TRUE LIMIT 1")->fetch_assoc();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Manage About Images</title>
...
<body>
    ...
    <div class="container mt-4">
        <h2>Manage About Section Images</h2>
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <h5 class="card-title">Current Active Image</h5>
                <?php if ($activeImage): ?>
                    <div class="text-center mb-4">
                        <img src="../<?= htmlspecialchars($activeImage['image_path']) ?>" 
                             class="img-fluid rounded" style="max-height: 400px;">
                        <p class="mt-2">Currently active image</p>
                    </div>
                <?php else: ?>
                    <div class="alert alert-warning">No active image selected</div>
                <?php endif; ?>
                
                <h5 class="card-title mt-4">All Images</h5>
                <div class="row">

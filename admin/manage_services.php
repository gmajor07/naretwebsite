...
// Handle Add or Update form actions
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_service'])) {
        ...
        $query = "INSERT INTO services_detail (title, description, image_path, category) VALUES (?, ?, ?, ?)";
        ...
        if ($_FILES['image']['size'] > 0) {
            ...
            $imagePath = handleImageUpload('image', '../assets/img/services/');
            $query = "UPDATE services_detail SET title=?, description=?, image_path=?, category=? WHERE id=?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("ssssi", $title, $description, $imagePath, $category, $id);
        } else {
            // Update without new image
            $query = "UPDATE services_detail SET title=?, description=?, category=? WHERE id=?";
            $stmt = $conn->prepare($query);
...
// Handle delete service (should be outside POST check)
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $query = "DELETE FROM services_detail WHERE id=?";
    $stmt = $conn->prepare($query);
...
// Fetch all services
$services = $conn->query("SELECT * FROM services_detail ORDER BY id DESC")->fetch_all(MYSQLI_ASSOC);
?>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="manage_services.php?delete=<?= $service['id']; ?>" 
